...
export default function FolderScreen() {
    ...
    const { items, createFolder, moveItems, renameItem, deleteItems, togglePin } = useFileSystem();
    ...
    if (!items) return <View><Text>Loading...</Text></View>;
    ...
    const renderSelectable = (component: React.ReactNode, id: string) => {
        ...
        return (
            <View key={id} style={{ position: 'relative', opacity: (isSelectionMode && !isSelected) ? 0.5 : 1 }}>
                {component}
                {isSelectionMode && (
    ...
    useFocusEffect(
        useCallback(() => {
            const unsubscribe = EventBus.onOpenSettings(() => setOptionsModalVisible(true));
            return () => unsubscribe();
        }, [])
    );
...
const styles = StyleSheet.create({
    ...
    sectionHeader: { fontSize: 14, fontWeight: '600', textTransform: 'uppercase', opacity: 0.5, marginLeft: 24, marginBottom: 10 },
    folderCard: {
        flexDirection: 'row',
        alignItems: 'center',
        marginHorizontal: 20,
