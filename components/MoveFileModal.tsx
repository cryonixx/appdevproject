...
import { FlatList, Modal, StyleSheet, Text, TouchableOpacity, View, useColorScheme } from 'react-native';
...
export default function MoveFileModal({ visible, onClose, folders, onSelectFolder, movingItems }: MoveFileModalProps) {
  ...
  const displayFolders = folders.filter(f => 
    f.folderId === currentPathId && 
    !movingItems.has(f.id)
  );
      ...
      <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={onClose}>
        <View style={[styles.container, { backgroundColor: themeColors.container }]}>
          ...
          <FlatList 
            ...
            ListEmptyComponent={<Text style={{padding:20, color:'#999', textAlign:'center'}}>No subfolders</Text>}
            renderItem={({ item }) => (
              <TouchableOpacity 
                style={[styles.folderItem, { borderBottomColor: themeColors.bordercolorSelected }]} 
                onPress={() => setCurrentPathId(item.id)} // Enter folder
              >
                <Ionicons name="folder" size={24} color={item.color || '#666'} />
                <Text style={[styles.folderName, { color: themeColors.text }]}>{item.name}</Text>
                <Ionicons name="chevron-forward" size={20} color={themeColors.lightext} />
              </TouchableOpacity>
            )}
          />
          ...
          <View style={styles.footer}>
            ...
            <TouchableOpacity 
              style={[styles.moveBtn, { backgroundColor: themeColors.container }]} 
...
const styles = StyleSheet.create({
  ...
  headerRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 15, paddingBottom:10, borderBottomWidth:1, borderColor:'#eee' },
  headerTitle: { fontSize: 18, fontWeight: 'bold', marginLeft: 10 },
  backButton: { padding: 5 },
  folderItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 15, borderBottomWidth: 1, gap: 15 },
